...
The emulated printer requires TrueType fonts to be setup to be able to print text. At a minimum it needs ``courier.ttf``.
...
The source files are recommendations, you are free to substitute them for suitable alternatives.
...
## Linux host
Create a directory ~/.config/dosbox-x/FONTS and copy or symlink the following files.
|===
|Original filename and location|New filename and location

|/usr/share/fonts/liberation-mono/LiberationMono-Regular.ttf|~/.config/dosbox-x/FONTS/courier.ttf
|/usr/share/fonts/liberation-serif/LiberationSerif-Regular.ttf|~/.config/dosbox-x/FONTS/roman.ttf
|/usr/share/fonts/liberation-sans/LiberationSans-Regular.ttf|~/.config/dosbox-x/FONTS/sansserif.ttf
|Download Ocraext.ttf|~/.config/dosbox-x/FONTS/ocra.ttf
|Download a Script (cursive) font|~/.config/dosbox-x/FONTS/script.ttf
|===
