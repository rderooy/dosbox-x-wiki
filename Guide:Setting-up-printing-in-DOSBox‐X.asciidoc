...
In DOSBox-X you can now simply redirect output to LPT1, or in DOS applications configure one of the printers listed above. For DOS programs that do not eject a form feed, you may need to manually eject a form feed (available in the DOS menu) before the output files are generated. 
