...
** will not allow for full SHARE support with Win32S
...
You do not have to enter anything for Description.
...
image::images/Windows:Windows_3.0_MIDI_2.png[Windows 3.0a MIDI, 600]
...
It installs and runs without problems. But be aware, that unless your running Windows 3.1x under real DOS, then DOSBox-X's integrated SHARE feature will be enabled by default which implements some of the DOS SHARE.EXE functions.
Win32s requires that SHARE.EXE is loaded, so your options are either the SHARE functions as implemented by DOSBox-X by default, or install Windows 3.1x in real DOS in DOSBox-X.

Do note that DOSBox-X's integrated SHARE feature does not fully implement the DOS SHARE.EXE functions, which may not be 100% safe for some applications, and Office 4.3, Visual C++ and Visual Basic are examples of programs that require DOS SHARE.EXE functions.
