...
It installs and runs without problems. But be aware, that unless your running Windows 3.1x under real DOS, then DOSBox-X's integrated SHARE feature will be enabled by default which partially implements the DOS SHARE.EXE functions.
Win32s requires that SHARE.EXE is loaded, so your options are either the partial SHARE functions, like DOSBox-X does by default, or install Windows 3.1x in real DOS in DOSBox-X.

Do note that DOSBox-X's integrated SHARE feature does not fully implement the DOS SHARE.EXE functions, which may not be 100% safe for some applications, and Office 4.3, Visual C++ and Visual Basic are examples of such programs.
