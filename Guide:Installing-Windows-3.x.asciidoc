...
* The above config uses 16MB RAM as that is the maximum supported by Windows 3.0, and should be more than enough for most Windows 3.x applications. But for Windows 3.1 and 3.11 you can go up to 256MB, as such for these Windows versions ``memsize=256`` is the largest safe value.
...
Unpack the archive, and you will find both a Windows 3.0 or 3.1 mouse driver in respectively the `windrv/dosboxpi/bin/win30` and `windrv/dosboxpi/bin/win31` directories.
...
Another option is the Windows 3.x mouse driver link:https://git.javispedro.com/cgit/vbados.git/about/[VBMOUSE.DRV] by @javispedro, including support for the scroll wheel.

If you are running Windows 3.x directly in the DOSBox-X integrated DOS environment, do not load the VBMOUSE.EXE driver. Only install the VBMOUSE.DRV in Windows 3.x.
...
== Known limitations
- When booting a guest OS, such an MS-DOS with Windows 3.x in DOSBox-X, so-called folder mounts are not supported.
