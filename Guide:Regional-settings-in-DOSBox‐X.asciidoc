...
Note: It is important to set the correct code page in the config file, via the ``COUNTRY=`` setting in [config] section of the config file, e.g. ``COUNTRY=42,852`` for the Czech language, and perhaps also the ``keyboardlayout`` config option mentioned in the next section.
