...
* When creating your HDD image with ``IMGMAKE``, instead of specifying a custom size, you can choose a pre-defined template. The advantage of using a pre-defined template in combination with an unsupported partition layout or filesystem is that you don't have to specify the sector-size, sectors, heads and cylinders each time you mount the HDD. The available pre-defined HDD templates can be seen by running ``IMGMAKE`` without arguments.
** Example: If you create a custom 20MB size HDD image (without partition or filesystem):
*** ``IMGMAKE hdd.img -t hd -size 20 -nofs``
*** ``MOUNT 2 hdd.img -size 512,32,2,640 -fs none``
** Example: If using a pre-defined HDD template:
*** ``IMGMAKE hdd.img -t hd_st225 -nofs``
...
Note: that until the partition is formatted, the use a a device number is necessary. Device number 2 represents the first disk on the primary HDD controller.
....
