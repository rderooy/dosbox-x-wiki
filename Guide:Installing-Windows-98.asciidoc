...
....
Then in DOSBox-X you need to create a new harddisk image file. At this time DOSBox-X's IMGMAKE command has no support for the FAT32 filesystem, although you can use a 3rd party tool to create FAT32 image files.

This FAT32 example uses a 4GB partition. While Windows 2000 or later won't let you format a volume bigger than 32GB with FAT32 using its built-in formatting tool, FAT32 can support up to 2TB as the maximum size. However, sizes larger then 128GB are not supported by the generic IDE driver in Windows 98. Larger sizes may be possible with 3rd party drivers, but are not covered here.
....
