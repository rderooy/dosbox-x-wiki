...
== Folder mounts
Starting with DOSBox-X 0.84.2, there is optional support to dynamically convert a host folder mount to an emulated FAT harddisk for use when you need to boot a guest OS, such as Windows 98.
...
NOTE: This conversion is dynamic, effectively creating an emulated FAT harddisk out of a folder mount. This converted FAT harddisk is not stored permanently.
...
The FAT type will be automatically selected based on the available disk-space on the host.
If there is more than 2GiB of space free (which is very likely on any modern system), it will convert it into a FAT32 disk, which in turn will require a guest OS with FAT32 support.

If you need FAT16 instead, you can use the `-convertfatro` option with the `boot` command. This will cause the emulated FAT HDD to be sized based on the size of the files in the folder with no free space.
As long as there is less then 2GiB of files, this should be an emulated FAT16 harddisk. However, this option will also make the emulated FAT harddisk read-only.
