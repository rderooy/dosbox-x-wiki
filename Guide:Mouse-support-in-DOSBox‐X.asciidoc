...
Relative mode works well in fullscreen mode, but starts to be a problem when running something like DOSBox-X in a window, when the mouse is not confined to the window.
...
As such, when running DOSBox-X in a window, using the integrated mouse support, it defaults to absolute mode.
...
- Running Windows inside DOSBox-X
...
== Capturing mouse
When running DOSBox-X in windowed mode, by default you can capture and release the mouse using CTRL-F10 (this key combination can be remapped in the key mapper).
...
NOTE: While capturing the mouse in windowed mouse is typically not needed, in some cases you will want to do it anyway, as leaving the DOSBox-X window with the mouse may cause gameplay problems.
...
As such, legacy DOS applications don't support it (as DOS and DOS applications at this time, generally were being sunset).

DOSBox-X has support for the Microsoft IntelliMouse with scroll wheel, but only from the integrated emulated DOS.
It currently does not work if you boot a guest OS.
To get the scroll wheel working, you will have to set `mouse_wheel_key=0`, as this option will otherwise interfere.

But since most DOS applications (and early Windows) don't support scroll wheel, DOSBox-X supports mapping the mouse wheel movements to keyboard keys.
By default DOSBox-X maps the mouse wheel to the up/down arrow keys.

The mouse wheel options can be found, when running in a window, from the menu bar under "Main" -> "Mouse wheel movements"
...
Set in the `[keyboard]` section, sets the type of PS/2 mouse attached to the AUX port.

valid settings:

- none: disables PS/2 mouse emulation
- 2button: emulate a Microsoft 2 button mouse
- 3button: emulate a Microsoft 3 button mouse
- intellimouse: emulate a MS IntelliMouse with scroll wheel
- intellimouse45: emulate a MS IntelliMouse with scroll wheel and 4th and 5th button

=== mouse_emulation
* Default value: locked
* Possible values: integration, locked, always, never

Set in the `[sdl]` section, determines whenn is mouse emulated ?
...
For the "integration" setting, the setting `integration device=true` must also be set in the `[cpu]` section.
...
Activated when the mouse is not locked.
...
However, any Microsoft mouse compatible mouse driver should work.
