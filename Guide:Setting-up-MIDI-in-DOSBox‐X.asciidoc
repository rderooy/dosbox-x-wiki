...
You can use ``aconnect -i`` to list available devices with ALSA. You can use the Client ID with device ID combined and set it for instance as ``midiconfig=14:0``
...
NOTE: If a game offers a "Sound Canvas", Roland SC-55, Roland SCC-1 or a Roland RAP-10 option you want to try, you need to use General MIDI emulation instead.
...
Unlike with the MT-32 emulation, General MIDI emulation is not provided by DOSBox-X itself, but rather by a separate software synthesizer.
DOSBox-X has support for a variety of such software synthesizers.

NOTE: There are also the *GS* extensions to General Midi by Roland, as used on the Roland Sound Canvas devices like the SC-55 and SC-88.
...
TIP: For some games, you may optionally want to download a SoundFont that implements the GS extensions. In that case look for a soundfont that emulates the Roland SC-55 or SC-88.
The Roland SC-55 or SC-88 (Sound Canvas), were was used by many musicians back in the day to compose General MIDI music for DOS and Windows games.

NOTE: While FluidSynth supports stacking soundfonts, this is not supported in this implementation.
So for instance, you cannot specify both FluidR3_GM and FluidR3_GS.
Instead you need a single soundfont that implements all the sounds you need.
...
You can use ALSA if you have a real MIDI sequencer, or a sound card with MIDI support. You can optionally also use ALSA with a software sequencer that creates a ALSA MIDI interface.
