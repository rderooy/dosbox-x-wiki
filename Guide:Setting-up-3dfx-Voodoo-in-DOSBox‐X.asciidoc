...
Then go back to the OpenGlide folder as detailed above, run make again, and it should then work.
This features.h file is not needed directly by OpenGlide but sometimes the macOS command line tools require it for the build script to complete successfully.
