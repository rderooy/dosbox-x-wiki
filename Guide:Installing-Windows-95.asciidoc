...
This FAT32 example uses a 4GB partition. While Windows 2000 or later won't let you format a volume bigger than 32GB with FAT32 using its built-in formatting tool, FAT32 can support up to 2TB as the maximum size. However, sizes larger then 128GB are not supported by the generic IDE driver in Windows 95. Larger sizes may be possible with 3rd party drivers, but are not covered here.
....
