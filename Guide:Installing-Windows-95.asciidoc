...
### Dynamic core
Starting with DOSBox-X 0.83.10 the dynamic_x86 core, which should perform much better, has been sufficiently enhanced that it can now be used for some use-cases with Windows 95.
To enable the dynamic core, merge the following settings into your config file:
...
If your application crashes DOSBox-X, please report it on the github issues page. One known issue is that opening a DOS window in Windows 95 will cause a crash when using ``core=dynamic_x86``.
In such cases, please switch back to ``core=normal`` until the issue is resolved.
