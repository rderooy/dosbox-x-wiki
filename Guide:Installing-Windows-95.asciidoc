...
However, should you run into problems such as application crashes or a Windows 95 blue-screen, please check if the same thing happens with ``core=normal``, and if not, please report the issue on the DOSBox-X Git issues page.
...
This is an example of a 16GB (16*1024=16384MB) disk, formatted as FAT32.
....
...
However, for Windows 95 it is necessary to force the IRQ to 5 to prevent Window 95 from complaining about a (non-existent) resource conflict.
