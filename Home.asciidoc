...
* **Decrease volume**: Decreases the sound volume of the current DOS session S.
...
* **Reported DOS version**: Changes the reported DOS version to the specified version from the list, including 3.3, 5.0, 6.22 and 7.1. Long filename (LFN) and FAT32 disk image support will be enabled when you select 7.1 as the reported DOS version.
...
* **Force load state mode**: Selects whether to load the saved state even if there is a mismatch in the DOSBox-X version and/or the program name.
...
+
DOSBox-X supports the save and load state feature including save slot support (with up to 10 save slots). Under the "Capture" menu you will see menu items including "Save state", "Load state", "Select save slot" and a "Force load state" toggle. Select a save slot you want to save to, and click "Save state", then the current state will be saved to the computer. When you want to load the state you can just click "Load state" from the menu; but if you want to select a different save slot to load, you need to select that save slot before loading the state. DOSBox-X will check for things like program name and memory size to make sure they match, and if they do not DOSBox-X may display a warning. These checks can be bypassed by enabling the "Force load state" toggle menu before you try to load the state.
