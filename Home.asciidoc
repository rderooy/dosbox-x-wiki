...
If you do need any sound output, you can also set the following to further improve the speed when DOSBox-X starts:
...
If you want to launch DOSBox-X to run some command(s) and then exit automatically without showing the DOSBox-X window at all, you can start DOSBox-X with the command-line option ``-silent`` to achieve this result.