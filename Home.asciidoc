...
+
Explains the ways to customize the regional settings in DOSBox-X, such as support for the Euro symbol and keyboard layouts.
* link:Guide%3ASetting-up-3dfx-Voodoo-in-DOSBox‐X[Guide: Setting up 3dfx Voodoo in DOSBox-X]
+
Explains the emulation of the 3dfx Voodoo using either the Voodoo hardware emulation or the Glide API passthrough.
* link:Guide%3ASetting-up-MIDI-in-DOSBox‐X[Guide: Setting up MIDI in DOSBox-X]
+
Explains how to set up and use MIDI emulations in DOSBox-X, such as General MIDI and Roland MT-32 synthesizer emulations.
* link:Guide%3ASetting-up-printing-in-DOSBox‐X[Guide: Setting up printing in DOSBox-X]
+
Explains how to use DOSBox-X's printing feature, either to a real printer or to a virtual printer (e.g. print to PNG or PostScript).
