...
DOSBox-X supports multiple host operating systems such as Windows, Linux and macOS (Mac OS X). Windows binaries (both 32-bit and 64-bit), Linux RPM packages (64-bit), macOS packages and special HX-DOS packages (for real DOS environments) are officially released periodically, typically in the end of a month or at the start of the next month. You can download zip packages for the supported platforms from the link:https://github.com/joncampbell123/dosbox-x/releases[Releases] page. All-in-one Windows installers for DOSBox-X are also available. You can use these self-installable packages if Windows is your platform, and you will be able to start DOSBox-X as soon as the installation ends. The Windows installer for DOSBox-X 0.83.3 (latest version) is available from: link:https://github.com/Wengier/dosbox-x-wiki/raw/master/DOSBox-X-0.83.3-setup.exe[DOSBox-X-0.83.3-setup.exe]
