...
All-in-one Windows installers for DOSBox-X are available to ease the installation process, which are especially recommended for new and non-expert users. You probably want to use these auto-installable packages if Windows is your platform, and you will be able to start DOSBox-X as soon as the installation ends. Also, a quick start guide is included in the package, and shell context menus can be automatically added for a fast launch of DOSBox-X from the Windows Explorer. The Windows installer for the latest official version of DOSBox-X (0.83.5 at this time) is available from: link:https://github.com/Wengier/dosbox-x-wiki/raw/master/DOSBox-X-Setup-Windows-latest.exe[DOSBox-X-Setup-Windows-latest.exe]
...
DOSBox-X features the Configuration GUI so that you can use its built-in graphical interface to modify its settings easily. An configuration file can be generated or updated by the Configuration GUI, or with the built-in CONFIG command (which can be found on the internal Z: drive when you run DOSBox-X).
...
You may instead want to do these (and many more) tasks from DOSBox-X’s GUI menu (see the link:DOSBox%E2%80%90X’s-GUI-Menus[DOSBox‐X’s GUI Menus] page for more information), so that there is no need to do them using these shortcuts in most cases.
...
* link:DOSBox%E2%80%90X’s-GUI-Menus[DOSBox‐X’s GUI Menus]
+
Gives detailed information about DOSBox-X's GUI menu system and its menu items.
...
+
Explains how to customize DOSBox-X's regional settings, such as support for the Euro symbol and keyboard layouts.
* link:Guide%3ASetting-up-a-Joystick-in-DOSBox‐X[Guide: Setting up a Joystick in DOSBox-X]
+
