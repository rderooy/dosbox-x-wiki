...
# Overview
DOSBox-X has support for the Novell NE2000 network adapter, but setting it up can be a bit convoluted. This guide hopes to simplify the process.
...
* The NE2000 IRQ is changed below from the default IRQ 3 to IRQ 10 to prevent conflicts with COM2 (this won't work if your emulating an original 8086 as those only support IRQ 0-7).
