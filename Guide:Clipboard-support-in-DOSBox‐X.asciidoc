...
Copying and pasting text to or from the Windows clipboard is also supported via the DOS API in DOSBox-X, so DOS programs that can directly access the Windows 3.x/9x clipboard running in DOSBox-X can also be used to copy and paste text to or from the Windows clipboard. For example, the https://www.horstmuc.de/div.htm#wclip[WCLIP tool] by Horst Schaeffer can be  used to communicate bidirectionally with the Windows clipboard within DOSBox-X. For example:
