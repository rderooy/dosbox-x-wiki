...
**Crash: The game crashes or misbehaves**

There can be many reasons that a game crashes or misbehaves. One of the more
common problems with older DOS games, is that they expect not to be loaded
too low in conventional (base) memory. So one of the first things to try is
to run ``LOADFIX`` before running the game. LOADFIX will by default allocate
64kB of RAM. e.g.
....
LOADFIX GAME.EXE
....
You can specify a different amount as follows:
....
LOADFIX -32 GAME.EXE
....
Will instead allocate 32kB.

If this does not fix the problem, try to search for the game on the following
website for known issues.
