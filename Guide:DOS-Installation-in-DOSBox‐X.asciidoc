...
== Hints and Tips
=== Host Folder Mounts
Starting with DOSBox-X 0.84.2, there is optional support to dynamically convert a host folder mount to an emulated FAT harddisk for use when you need to boot a guest OS, such as PC DOS, MS-DOS or FreeDOS.
...
=== Boot menu
If you used ``IMGMAKE`` from DOSBox-X prior to 0.84.2 to create a partitioned image, when booting from the image you will get the following boot menu with a 4-second timeout.
...
Images created with `IMGMAKE` from DOSBox-X 0.84.2 or later will no longer show this menu.
