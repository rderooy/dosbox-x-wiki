...
This made sense back in the day, but it provides no speed advantages when run on an emulator on a modern host operating system. In fact, it may actually be counterproductive.
...
NOTE: If SMARTDRV is being used, you MUST do a ``SMARTDRV /C`` *before* invoking the ``SHUTDOWN`` command as otherwise you risk filesystem corruption.
