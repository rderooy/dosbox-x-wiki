...
1. There is a setting "ver" under the [dos] section in DOSBox-X's config file (dosbox-x.conf by default), or from the DOSBox-X Configuration GUI. For example, setting "ver = 7.1" will cause DOSBox-X to set the initial DOS version as 7.1, instead of the default 5.0. Note that LFN (long filename) support will be enabled when the reported version is set to 7.0 or higher with the default setting.
2. Alternatively, you can set the reported DOS version with the ``VER`` command in the DOSBox-X command line. For example, ``VER SET 6.22`` will cause DOSBox-X to claim to be version 6.22. Also note that ```VER SET 7.1``` will set the reported DOS version to 7.1 (7.10) whereas ```VER SET 7 1``` will set the reported DOS version to 7.01.
