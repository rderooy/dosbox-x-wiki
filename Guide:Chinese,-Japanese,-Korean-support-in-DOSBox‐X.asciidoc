...
There are some TTF config options related to CJK support, such as automatic box-drawing recognition and SBCS <=> DBCS mode switch. These can also be toggled dynamically from the drop-down menu (inside "Video" => "TTF options").

== PC-98 mode ==

DOSBox-X provides emulation for the Japanese NEC PC-98 mode, so that you can play PC-98 games or programs in it. The PC-98 mode can be started with the setting ``machine=pc98``, and it can also work in combination with the TrueType font output. See the link:Guide%3APC‐98-emulation-in-DOSBox‐X{suffixappend}[PC‐98 emulation guide page] for detailed information about PC-98 emulation in DOSBox-X.

== AX (JEGA) mode ==

DOSBox-X also provides emulation for the Japanese AX (JEGA) mode, which can be started with the setting ``machine=jega``, and can also work in combination with the TrueType font output. In this mode you can press the right Ctrl key to toggle between romaji and kana input mode.
