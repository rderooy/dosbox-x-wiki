...
+
DOSBox-X supports many additional code pages for conversions between the host and guest systems, both for the TrueType font (TTF) output and other outputs. For the TrueType font output, DOSBox-X also supports DBCS code pages used by Chinese, Japanese, and Korean languages, so with a suitable TTF font you will be able to run text-mode DOS applications in these languages using the output. 
