...
NOTE: ``IMGMAKE`` will create fixed-size VHD image files, if the filename extension is `.vhd`. ``IMGMAKE`` cannot create dynamic VHD image files.
...
For example to create a 40 MiB VHD image using link:https://www.qemu.org/download/[qemu-img]:
...
The disadvantage is that only fixed size VHD images can be created this way. If you want dynamic images, another method will have to be used.
