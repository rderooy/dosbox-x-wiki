...
And they may optionally be prefixed with the host path to the file. e.g. ``disk.ima`` may become ``"C:\Users\John Doe\dosroot\disk.ima"``.
...
NOTE: In addition to the below DOSBox-X command line utility, it is possible to create diskette images at any time from the DOSBox-X menu.
...
....
NOTE: DOSBox-X does not supply a FORMAT command, so creating diskette images that are not formatted is typically not recommended.
...
A diskette image that is not formatted, or has an unsupported filesystem can be attached as follows:
...
==== Mounting an empty floppy disk drive
Starting with DOSBox-X 2024.03.01 it is possible to mount an empty floppy drive.
This is really only if you intend to boot a guest OS, and want to be able to insert a diskette in the drive after booting using the DOSBox-X drop-down menu's.

When no `-size` option is specified, a 3.5" 1.44MB drive type is emulated.
...
To emulate an empty 5.25" 1.2MB drive as the first floppy drive:
...
[cols="1,1,4"]
|===
| Drive type        | `-size` option | Notes

| SS/DD 5.25" 160KB | 512,8,1,40     | Only really for DOS 1.x backward compatibility
| SS/DD 5.25" 180KB | 512,9,1,40     |
| DS/DD 5.25" 320KB | 512,8,2,40     | Only really for DOS 1.x backward compatibility
| DS/DD 5.25" 360KB | 512,9,2,40     | Can also R/W 5.25" SS/DD media
| DS/HD 5.25" 1.2MB | 512,15,2,80    | Can also R/W all other 5.25" media
| DS/DD 3.5" 720KB  | 512,9,2,80     | Can also R/W 3.5" SS/DD media
| DS/HD 3.5" 1.44MB | 512,18,2,80    | Can also R/W 3.5" SS/DD and DS/DD media
| DS/ED 3.5" 2.88MB | 512,36,2,80    | Can also R/W all other 3.5" media
|===

NOTE: The two most common and useful drive types to emulate are the 5.25" 1.2MB and the 3.5" 1.44MB varieties as they can each read and write lower capacity variants of the same physical media size.

NOTE: The current DOSBox-X release seems to have a bug where Win9x will only see the first floppy drive.
...
NOTE: In addition to the below DOSBox-X command line utility, it is possible to create harddisk images at any time from the DOSBox-X menu.
Go to the "DOS" menu, and select "Create blank disk image…".
This option allows a few common HDD sizes to be created, for less common types you need to use the command line utility.

DOSBox-X has a built-in ``IMGMAKE`` command with which you can create partitioned and FAT formatted harddisk images in either RAW MFM sector image or VHD format from the DOSBox-X command prompt.
...
NOTE: As of DOSBox-X 2023-10-06, you can specify a wildcard instead of a drive letter (e.g., `imgmount *: hdd.img`). This will cause DOSBox-X to take the next available drive letter.
...
A harddisk image that is not yet partitioned needs additional parameters, such as a template or the size in SHC geometry.
...
Or using the original SHC geometry it was based on:
...
- In DOSBox-X 2022.08.00 the IDE emulation was enhanced such that link:https://en.wikipedia.org/wiki/SafeDisc[SafeDisc] V1 games should work
- In DOSBox-X 2022.09.00 another minor change was introduced to solve one of the issues with SafeDisc V2
...
=== Mounting an empty CD-ROM drive
Starting with DOSBox-X 2024.03.01 it is possible to mount an empty CD-ROM drive.
This is useful for when you intend to boot a guest OS, and want to be able to insert a CD-ROM in the drive after booting using the DOSBox-X drop-down menu's.

[source, console]
....
imgmount e -t iso -empty
....
...
== FDISK
DOSBox-X does not supply the FDISK utility to partition harddisks. Instead it supplies an `IMGMAKE` utility which can be used to create HDD image files, including paritioning and formatting them.

If however you really want to run the DOS FDISK utility from the DOSBox-X integrated emulated DOS, it is possible as of DOSBox-X 2023.10.06.

First create a blank HDD image file:
....
Z:\>IMGMAKE hdd.img -t hd -nofs -size 100
Creating image file "hdd.img" with 812 cylinders, 4 heads and 63 sectors
....

Then attach the blank image using the CHS geometry. '2' is a drive number that refers to the first HDD.
....
Z:\>IMGMOUNT 2 hdd.img -size 512,63,4,812
Drive number 2 mounted as hdd.img
....

Now running `FDISK` from MS-DOS or PC DOS should work, and allow partitions to be created or deleted.

NOTE: If FDISK return an error "Incorrect DOS version", you must set the DOSBox-X reported DOS version to match the DOS version that FDISK came from.
e.g., if FDISK.EXE came from MS-DOS 6.22, first run `VER SET 6.22`.


== FORMAT
DOSBox-X does not supply the FORMAT utility to format floppies or harddisks. Instead it supplies an `IMGMAKE` utility which can be used to create Floppy or HDD image files, including formatting them.

If however you really want to run the DOS FORMAT utility from the DOSBox-X integrated emulated DOS, it is partly possible as of DOSBox-X 2023.10.06.

NOTE: A harddisk image MUST be partitioned before it can be formatted.

NOTE: Formatting an unformatted floppy disk image is not possible at this time using this method. But it is possible to format a floppy disk that is already formatted.

NOTE: It is important to mount the image file on a **drive letter**, as `FORMAT` cannot deal with drive numbers.

In case of a harddisk, first follow the steps in the FDISK section above to create the image file and partition it.

Then attach the image using `IMGMOUNT`.

NOTE: For harddisk images, once the image is partitioned it is no longer necessary to specify the geometry.

....
Z:\>IMGMOUNT c hdd.img
...
Once the image file is mounted on a drive letter, running `FORMAT` from MS-DOS or PC DOS should work.

NOTE: the '/s' option to transfer system files does not work properly at this time.
...
There is limited support for changing the loaded Floppy or CD image from the DOS menu where you will find the options "Change current floppy image..." and "Change current CD image...".
...
* Requires either an image to be mounted before you boot a Guest OS, or an empty drive to be emulated. Otherwise, you will get an error such as "No floppy drive is currently available".
