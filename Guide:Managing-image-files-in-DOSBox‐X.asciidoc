...
** No support in DOSBox-X for VHDX or VHD dynamic options
...
== VHD support
DOSBox-X has limited support for fixed-size VHD (aka Virtual PC) hard disk images. Dynamic sized VHD, or the newer VHDX format, are **not** supported.

DOSBox-X does not provide a way to create VHD image files. One verified working method is to use link:https://www.qemu.org/download/[qemu-img].
For example to create a 40MiB HDD image:

[source, console]
....
qemu-img create -f vpc -o subformat=fixed hdd.vhd 40M
....

At this point, as the image file will not be partitioned or formatted, it can only be mounted as such:
...
Once a VHD file has been partitioned and formatted, it can be mounted just like a RAW MFM image file.
